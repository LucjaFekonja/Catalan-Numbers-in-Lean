...
theorem binom_div_eq (n : ℕ) (h : 0 < n) : (Nat.choose (2 * n) n) / (n + 1) = (Nat.choose (2 * n) (n + 1)) / n := by
...
-- theorem binom_div_eq_sub_binom (n : ℕ) (h : 0 < n) : (Nat.choose (2 * n) n) / (n + 1) = (Nat.choose (2 * n) n) - (Nat.choose (2 * n) (n + 1)) := by
--  rw []
